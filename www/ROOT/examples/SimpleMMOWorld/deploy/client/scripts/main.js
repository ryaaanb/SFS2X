(function(){
	...
	this.init = function()
	{
		...
		loadQueue.loadManifest([
	    	...
	    	{id: "ssAvatar", src:"images/spritesheet_avatar.png"},
				{id: "ssAvatarHead", src:"images/spritesheet_avatarhead.png"},
	...
	/*this.onCurtainSliderChange = function(e)
	{
		drawCurtain();
	}*/
	...
	function onRoomJoin(evtParams)
	{
		...
		createAvatar(sfs.mySelf, accessX, accessY, dir, head);
	...
	function onProximityListUpdate(evtParams)
	{
		...
		for (var ai = 0; ai < added.length; ai++)
		{
			...
			// ITEMS
			var head = au.getVariable(USERVAR_HEAD).value;

			// Create avatar
			createAvatar(au, vec.px, vec.py, dir, head);
	...
	function onPreloadComplete()
	{
		...
		ssAvatar = new createjs.SpriteSheet({
											...
											// Create avatar spritesheet
		ssAvatarHead = new createjs.SpriteSheet({
			images: [loadQueue.getResult("ssAvatarHead")],
			frames: {width: 30, height: 68, regX: 15, regY: 66},
			animations: {
				avatarE: [0, 19, "avatarE"],
				avatarN: [20, 38, "avatarN"],
	...
	function setView(viewId, error)
	{
		...
		else if (viewId == "game")
		{
			...
			sprite.regX = 30;
			sprite.regY = 37;
			addSprite(sprite, 651, 500, false);
			...
			sprite.regX = 30;
			sprite.regY = 68;
			addSprite(sprite, 800, 650, false);
	 ...
	 */
	function createAvatar(user, x, y, dir, head)
	{
		// Instantiate avatar
		var avatar = new Avatar(user.id, user.name, ssAvatar, ssAvatarHead, ssBalloon);
		...
		// Show viewport and Area of Interest boundaries
		if (user.isItMe)
		{
		...
		// Draw curtain masking area outside of viewport
		//if (user.isItMe)
			//drawCurtain();
	}
	...
	{
		var avatar = getAvatar(sfs.mySelf.id);
		var curtainAlpha = $('#curtainSl').jqxSlider('getValue') / 100;

		if (avatar != null && curtainAlpha > 0)
		{
			var vpBounds = avatar.getViewportBounds();
			var g = curtain.graphics;
			
			g.clear();

			g.beginFill("rgba(238,238,238," + curtainAlpha + ")");

			g.lineTo(sfs.lastJoinedRoom.higherMapLimit.px, 0);
			g.lineTo(sfs.lastJoinedRoom.higherMapLimit.px, sfs.lastJoinedRoom.higherMapLimit.py);
			g.lineTo(0, sfs.lastJoinedRoom.higherMapLimit.py);
			g.lineTo(avatar.x + vpBounds.x, avatar.y + vpBounds.y + vpBounds.height);
			g.lineTo(avatar.x + vpBounds.x + vpBounds.width, avatar.y + vpBounds.y + vpBounds.height);
			g.lineTo(avatar.x + vpBounds.x + vpBounds.width, avatar.y + vpBounds.y);
			g.lineTo(avatar.x + vpBounds.x, avatar.y + vpBounds.y);
			g.lineTo(avatar.x + vpBounds.x, avatar.y + vpBounds.y + vpBounds.height);
			g.lineTo(0, sfs.lastJoinedRoom.higherMapLimit.py);
			g.lineTo(0, 0);
